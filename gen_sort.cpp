...
* Peng Liu 12/07/02 Modified Barclay's code to generate 100-bytes records
instead of 101-bytes records.
* JNG 01/19/04 Modified Peng Liu code to use long long (64bit int) to handle
more than 2 billion records.
* Rong Chen 07/31/08 Port JNG's code to linux
*
* Historical NOTE:
* LIMITATIONS:
* Program works up to 2^31 records (about 200GB), before 31 bit record counter
overflows.
* We hope this will be a problem someday.
*
* This was a problem in 2004, and the problem was removed by using long-long
datatype.
*/
#include <stdlib.h>
#include <stdio.h>
...
enum {
    rec_len = key_len + recno_len + pad_len
};
enum {
    buf_nrec = 1 << 4
};
enum {
    buf_size = buf_nrec * 100
};
...
fill_buf(char *buf) {
    ...
    for (cnt = 0; cnt < buf_nrec; cnt++) {
    /* Fixed times of buf got filled */
    /* ATTENTION: The author is confusing an index variable and a counter variable deliberately */
        if ((cur_rec + cnt + 1) > rec_max) {
            break;
        }
        gen_rec(rec + cnt);
    }
